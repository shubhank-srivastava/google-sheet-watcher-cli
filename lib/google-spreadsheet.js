...
var GoogleSpreadsheet = function( ss_key, auth_id, options ){
  ...
  this.setSpreadsheetKey = function(key){
    ss_key = key;
  }
  ...
  this.getSpreadsheetList = function(cb){
    self.makeFeedRequest(["spreadsheets"], 'GET', null, function(err, data, xml){
      if ( err ) return cb( err );
      if (data===true) {
        return cb(new Error('No response to getInfo call'))
      }
      var worksheets = forceArray(data.entry);
      var ss_data = worksheets.map(function(ws){
        return {
          id: ws.id.slice(ws.id.lastIndexOf('/')+1),
          url: ws.id,
          title: ws.title,
          updated: ws.updated
        }
